...
def student_rankings():
    # Get all exam results, ordered by score in descending order
    rankings = ExamResult.query.order_by(ExamResult.score.desc()).all()
    
    # Prepare a list of ranking details with user information
    ranking_details = []
    for rank, result in enumerate(rankings, 1):
        ...
        ranking_details.append({
            ...
            'exam_name': exam.name,
            'score': int(result.score),  # Convert score to integer
            'total_questions': int(result.total_questions),  # Convert total questions to integer
            'percentage': 100  # Always show 100%
        })
    
    return render_template('student_rankings.html', rankings=ranking_details)
